{------------------------------------------------------------------------------}
{ Changer    |  Date    |        Contents       |          Description         }
{------------------------------------------------------------------------------}
{shalamyansky|2023-03-15| Load methods extended | to support AnsiString reading}
{------------------------------------------------------------------------------}
...
//    procedure SetDocType(const Value: IXMLDocumentType);
  ...
  public
    ...
    //shalamyansky 2023-03-15 - old interface for new implementation
    function LoadFromStream( const Stream:TStream ):Boolean; overload;
    //shalamyansky 2023-03-15 - new interface with CodePage support
    function LoadFromStream( const Stream:TStream; CodePage:LONGWORD ):BOOLEAN; overload;
    ...
    //shalamyansky 2023-03-15
    function LoadAnsiXML( const XML:AnsiString ):BOOLEAN;
    property ParseError: IXMLParseError read GetParseError;
  end;
...
//shalamyansky 2023-03-15
function TXMLDocument.LoadAnsiXML( const XML:AnsiString ):BOOLEAN;
var
    Stream: TMemoryStream;
begin
    Result := FALSE;
    ...
    try
        Stream := nil;
        Stream := TMemoryStream.Create;
        Stream.Write( PAnsiChar( POINTER( XML ) )^, Length( XML ) );
        Stream.Seek( 0, soFromBeginning );
        Result := LoadFromStream( Stream, 0 {CP_ACP} );
    finally
        Stream.Free;
        Stream := nil;
    end;
...
begin
    Result := LoadfromStream( Stream, CP_UTF8 );
end;{ TXMLDocument.LoadFromStream }

//shalamyansky 2023-03-15 - new interface with CodePage support
function TXMLDocument.LoadFromStream( const Stream:TStream; CodePage:LONGWORD ):BOOLEAN;
var
  XTS: TXMLTextStream;
...
//XTS := TXMLTextStream.Create(Stream, smRead, TEncoding.UTF8, False);
  Encoding := nil;
  Encoding := TEncoding.GetEncoding( CodePage );                 //shalamyansky 2024-02-01
  XTS := TXMLTextStream.Create(Stream, smRead, Encoding, False); //shalamyansky 2023-03-15
  try
    try
    ...
    FreeAndNil(XTS);
    FreeAndNil( Encoding ); //shalamyansky 2024-02-01
    FURL := '';
  end;
end;{ LoadFromStream }
